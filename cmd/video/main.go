...
import (
	...
	video "douyin-demo-micro/kitex_gen/video/videoservice"
...
func main() {
	...
	addr, err := net.ResolveTCPAddr("tcp", "127.0.0.1"+util.VideoServicePort)
	if err != nil {
		panic(err)
	}

	util.InitJaeger(util.VideoService)
	rpc.InitRPC()
	err = dal.InitDB()
	if err != nil {
		panic(err)
	...
	svr := video.NewServer(new(VideoServiceImpl),
		server.WithServerBasicInfo(&rpcinfo.EndpointBasicInfo{ServiceName: util.VideoService}), // server name
